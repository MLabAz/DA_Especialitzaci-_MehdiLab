...
SELECT cp.company_name ,avg(tr.amount) as MitjaMesGranVendes FROM company cp INNER JOIN transaction tr ON cp.id = tr.company_id GROUP BY cp.id, cp.company_name ORDER BY avg(tr.amount) DESC LIMIT 1 ;
...
SELECT date(tr.timestamp) ,sum(tr.amount) TotalVentes FROM transaction tr WHERE tr.declined = 0 GROUP BY date(tr.timestamp) ORDER BY TotalVentes DESC limit 5; 

/*Exercici 2
Quina és la mitjana de vendes per país? Presenta els resultats ordenats de major a menor mitjà.*/
SELECT cp.country, avg(tr.amount) MitjaDeVendes FROM transaction tr INNER JOIN company cp ON cp.id = tr.company_id GROUP BY cp.country ORDER BY avg(tr.amount) DESC;
...
per la qual cosa et demanen la informació sobre la quasntitat de transaccions que realitzen les empreses,
...
*/
SELECT tr.company_id, cp.company_name , count(*) numTransacions, CASE 
        WHEN count(*) >= 3 THEN "+ de 4"
        ELSE "- de 4" 
        END MoreThan4
        FROM transaction tr JOIN company cp ON tr.company_id = cp.id GROUP BY tr.company_id, cp.company_name;
